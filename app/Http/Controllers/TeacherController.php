...
class TeacherController extends ResponseController
{
 ...
 //////// get lectures
    ...
    {
        $studentmark=Valuation::where('teacher_id',$request->user('employee')->id)->select('lecture')->distinct()->get();
...
///////// set Valuation
    ...
    public function setValuation(Request $request)
    {
        ...
        $user=Student::find($request->student_id)->first();
         if($user->fcm_token){
         FCMService::send(
            $user->fcm_token,
            [
                'title' => 'تقييمات',
                'body' => 'تم اضافة تقييم لك',
                'sound' => true
            ]
        );}
