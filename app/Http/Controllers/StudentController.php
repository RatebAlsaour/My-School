...
class StudentController extends ResponseController
{
    ...
    public function getMark(Request $request)
    {
        ...
        $studentmark = Mark::where('student_id', $request->user('student')->id)->where('created_at', '>', $class->created_at)->orderby('type')->orderby('created_at', 'desc')->get();
    ...
    {
        $class = ClassRoom::find($request->user('student')->class_room_id)->first();
        $student_note = Student_Note::where('student_id', $request->user('student')->id)->where('created_at', '>', $class->created_at)->orderby('created_at', 'desc')->get();
        $list = [];
    ...
    public function getNoteClass(Request $request)
    {
        ...
        $notes = Note::whereIn('id', $list)->orderby('created_at', 'desc')->get();
        return $this->responseData($notes, 'succeed');
    }
    ...
    {
        $class = ClassRoom::find($request->user('student')->class_room_id)->first();
        $valuations = Valuation::where('student_id', $request->user('student')->id)->where('created_at', '>', $class->created_at)->orderby('created_at', 'desc')->get();
    ...
    {
        $class = ClassRoom::find($request->user('student')->class_room_id)->first();
        $student = Absence_Let_st::where('student_id', $request->user('student')->id)->where('created_at', '>', $class->created_at)->orderby('created_at', 'desc')->get();
        if (!$student) {
    ...
    {
        $student = Student::where('id', $request->user('student')->id)->with('classRoom')->first();
    ...
    public function getTimeTable(Request $request)
    {
        ...
        if (!$class->image) {
            return $this->respo([], 'not exists yet');
        }
    ...
    public function getMyFiles(Request $request)
    {
        ...
        $studefile = StudyFile::whereIn('id', $list)->orderby('created_at', 'desc')->get();
