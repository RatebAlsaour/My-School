...
/////////////// get subjects
    ...
    public function getsubgects(Request $request)
    {
        $studentmark=Mark::select('subject')->distinct()->get();
    ...
    public function getTypeExam(Request $request)
    {
        $studentmark=Mark::select('type')->distinct()->get();
        return $this->responseData($studentmark,'successful');
    }
...
////////// set mark for class
    ...
    public function setMarkForClass(Request $request)
    {
        ...
        [
         ...
         $user=Student::where('id',$request->student_id)->first();
         if($user->fcm_token){
         FCMService::send(
            $user->fcm_token,
            [
                'title' => 'علامات',
                'body' => 'تم اضافة علامة جديدة',
                'sound' => true
            ]
        );}
    ...
    public function addFileToClass(Request $request)
    {
        ...
        [
         ...
         if($validate->fails()){
           ...
           $size=FacadesFile::size(storage_path('app/Files/').''.$file)/1024;
           if($size<1024){
               $s=round($size,2);
               $size=$s.''.'KB';
           }
